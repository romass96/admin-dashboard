...
export default {
  ...
  async mounted() {
    ...
    if (sourceId) {
      ...
      const source = this.findProductById(sourceId);
      this.productName = source.name + '-copy';
      this.productPrice = source.price;
      this.productDescription = source.description;
      this.productCategory = source.category.id;
    }
  },
  methods: {
    createProduct() {
